...
class VersionAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        response = requests.get(os.path.join(os.getenv('LAUNCHER_URL'), "version"))
        if response.status_code != 200:
            raise RuntimeError('incorrect result from \'version\' service: %s' % response.text)
        print("Server version: %s" % response.text)
        sys.exit(1)
...
exec_arguments = [
    ...
    ['-c', '--cpus', {"type": int,
                      "help": 'number of cpus - if not provided, '
...
parser_exec = subparsers.add_parser('exec',
                                    help='execute a generic docker-utility task on the service '
                                         'associated to provided options')
...
def process_request(serviceList, cmd, subcmd, is_json, ARGS, auth=None):
    ...
    if cmd == "service" and subcmd == "list":
        params = {'all': ARGS.all}
        response = requests.get(os.path.join(ARGS.url, "service/list"), auth=auth, params=params)
        if response.status_code != 200:
        ...
        response = requests.get(os.path.join(ARGS.url, "task/list", ARGS.prefix + '*'), auth=auth,
                                params={"with_parent": ARGS.parent, "service": ARGS.service,
                                        "status": ARGS.status})
        if response.status_code != 200:
            raise RuntimeError('incorrect result from \'task/list\' service: %s' % response.text)
        result = response.json()
        ...
        response = requests.get(os.path.join(ARGS.url, "service/describe", ARGS.service), auth=auth)
        if response.status_code != 200:
            raise RuntimeError(
                'incorrect result from \'service/describe\' service: %s' % response.text)
        res = response.json()
        ...
        response = requests.get(os.path.join(ARGS.url, "service/check", ARGS.service),
                                json=options, auth=auth)
        if response.status_code != 200:
            raise RuntimeError(
                'incorrect result from \'service/check\' service: %s' % response.text)
        res = response.json()
        ...
        response = requests.post(launch_url,
                                 ...
                                 auth=auth)
        if response.status_code != 200:
            raise RuntimeError('incorrect result from \'task/launch\' service: %s' % response.text)
        result = response.json()
        if not is_json:
        ...
        response = requests.get(os.path.join(ARGS.url, "task/status", ARGS.task_id), auth=auth)
        if response.status_code != 200:
            raise RuntimeError('incorrect result from \'task/status\' service: %s' % response.text)
        result = response.json()
        if not is_json:
            times = []
        ...
        if not is_json:
            ...
            if confirm():
                for k in result:
                    response = requests.delete(os.path.join(ARGS.url, "task", k["task_id"]),
                                               auth=auth)
                    if response.status_code != 200:
                        raise RuntimeError(
                            'incorrect result from \'delete_task\' service: %s' % response.text)
    elif cmd == "task" and subcmd == "terminate":
        response = requests.get(os.path.join(ARGS.url, "task/terminate", ARGS.task_id), auth=auth)
        if response.status_code != 200:
            raise RuntimeError('incorrect result from \'task/terminate\' service: %s'
                               % response.text)
        result = response.json()
        if not is_json:
            res = result["message"]
    elif cmd == "task" and subcmd == "file":
        response = requests.get(os.path.join(ARGS.url, "task/file", ARGS.task_id, ARGS.filename),
                                auth=auth)
        if response.status_code != 200:
            raise RuntimeError('incorrect result from \'task/file\' service: %s' % response.text)
        res = response.content
    elif cmd == "task" and subcmd == "log":
        response = requests.get(os.path.join(ARGS.url, "task/log", ARGS.task_id), auth=auth)
        if response.status_code != 200:
            raise RuntimeError('incorrect result from \'task/log\' service: %s' % response.text)
...
if __name__ == "__main__":
    ...
    try:
        RES = process_request(SERVER_LIST, ARGS.cmd, ARGS.subcmd, ARGS.display == "JSON", ARGS)
